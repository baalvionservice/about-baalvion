...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <aside className="w-64 border-r border-gray-200 bg-white hidden lg:flex flex-col fixed inset-y-0 shadow-sm overflow-y-auto custom-scrollbar">
        ...
        <div className="p-4 border-t border-gray-100 space-y-1 sticky bottom-0 bg-white">
          <button className="flex w-full items-center gap-3 px-4 py-2 rounded-md text-xs font-bold text-gray-500 hover:bg-gray-50 hover:text-gray-900">
            <Settings className="w-4 h-4" />
            System Settings
          </button>
          <button onClick={handleLogout} className="flex w-full items-center gap-3 px-4 py-2 rounded-md text-xs font-bold text-destructive hover:bg-destructive/5">
      ...
      <main className="flex-1 lg:ml-64">
        <header className="h-16 border-b border-gray-200 bg-white/80 backdrop-blur-md px-8 flex items-center justify-between sticky top-0 z-40">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-9 h-9 rounded-full bg-orange-50 border border-orange-100 flex items-center justify-center text-[#FF9900] font-bold shadow-sm">B</div>
          </div>
        </header>
